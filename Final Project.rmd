...
regsubsets.out <- regsubsets(overall_rating ~  age + height_cm +weight_kgs + 
                           ...
                           marking + standing_tackle +sliding_tackle   ,data=training_data, nvmax=10) 
...
fit1 <- lm(log(overall_rating) ~ age + height_cm + log(value_euro) + wage_euro + international_reputation.1.5. + national_rating + crossing + finishing + heading_accuracy + short_passing + volleys + dribbling + curve + freekick_accuracy + long_passing + ball_control + acceleration + sprint_speed+agility +reactions + balance +shot_power +jumping + stamina +strength + long_shots + interceptions +vision + positioning +penalties + composure + marking + standing_tackle +sliding_tackle   ,data=training_data)
